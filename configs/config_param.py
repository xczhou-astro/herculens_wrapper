...
def lens_mass_config(image_size=None, pixel_scale=None, args=None):
    ...
    # num_gaussians = 3
    # lens_mass_type_list = ['EPL'] + ['GAUSSIAN'] * num_gaussians + ['SHEAR']
    
    if lens_mass_type_list == ['SIE', 'SHEAR']:
        lens_mass_params_list = [
            {
                'theta_E': [0.4, 0.1, 0.3, 0.55],
                'e1': [0.0, 0.1, -0.5, 0.5],
                'e2': [0.0, 0.1, -0.5, 0.5],
                'center_x': 0.0,
                'center_y': 0.0,
    ...
    elif lens_mass_type_list == ['EPL'] + ['GAUSSIAN'] * num_gaussians + ['SHEAR']:
        lens_mass_params_list = [
            {
                'theta_E': [0.4, 0.1, 0.3, 0.6],
                'gamma': [2.0, 0.1, 1.0, 2.5],
                'e1': [0.0, 0.1, -0.5, 0.5],
                'e2': [0.0, 0.1, -0.5, 0.5],
                'center_x': 0.0,
        ...
        min_sigma = args.pixel_scale
        max_sigma = args.pixel_scale * args.crop_size / 2.0
        sigma_list = 10**(np.linspace(np.log10(min_sigma), np.log10(max_sigma), num_gaussians))
        for k in range(num_gaussians):
            init_sigma = sigma_list[k]
            unc_sigma = 0.5 * init_sigma
            lens_mass_params_list.append(
                {
                    'amp': [1.0, 0.1], 
                    'sigma_x': [init_sigma, unc_sigma, min_sigma, max_sigma],
                    'sigma_y': [init_sigma, unc_sigma, min_sigma, max_sigma],
...
def source_light_config(image_size=None, pixel_scale=None, args=None, 
    ...
    num_gaussians = 2
    # source_light_type_list = ['SERSIC_ELLIPSE'] + ['GAUSSIAN_ELLIPSE'] * num_gaussians
        ...
        source_light_params_list = [
            {
                ...
                'e1': [0.0, 0.1, -1.0, 1.0],
                'e2': [0.0, 0.1, -1.0, 1.0],
                'R_sersic': [0.2, 0.1, 0.01, 1.0],
                'n_sersic': [2.0, 0.5, 0.1, 10.0],
                'center_x': [0.0, 0.1, -0.3, 0.3],
                'center_y': [0.0, 0.1, -0.3, 0.3],
            },
            {
                'amp': [1.0, 0.1],
                'sigma': [0.05, 0.01, 0.01, 0.3],
                'center_x': [0.0, 0.1, -0.3, 0.3],
                'center_y': [0.0, 0.1, -0.3, 0.3],
            }
            ]
    ...
    elif source_light_type_list == ['SERSIC_ELLIPSE'] + ['GAUSSIAN_ELLIPSE'] * num_gaussians:
        min_sigma = 0.03
        max_sigma = args.crop_size * pixel_scale / 2.0
        sigma_list = 10**(np.linspace(np.log10(min_sigma), np.log10(max_sigma), num_gaussians))
