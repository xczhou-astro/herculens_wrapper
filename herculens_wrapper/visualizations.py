...
def plot_lens_light_subtracted_image(
    ...
    if noise_map is not None:
        ...
        ax[2].set_title(f'Data - Lens light (S/N; $\\chi^2$ = {chi2_subtracted:.2f})')
...
def plot_ring_model_comparison(
    ...
    panels = [
        ...
        (residual, f'Residuals ($\\chi^2$ = {chi2:.2f})', 'bwr'),
    ]
...
def _get_mge_exclude_list(all_names, threshold=3):
    ...
    for prefix, indices in groups.items():
        ...
        # (e.g. m=1, 3, 4).  Their parameters then look superficially like a
        # multi-Gaussian sequence (``lens_a_m_1``, ``_2``, ``_3``), but they
        # are distinct physical multipoles and must remain in every SVI/HMC
        # corner plot.
        local_prefix = prefix.rsplit('/', 1)[-1]
        if local_prefix.endswith((
            'lens_e_x', 'lens_e_y', 'lens_a_m', 'lens_phi_m',
            'lens_delta_phi_m',
        )):
            continue
...
def lens_mass_ellipticity_summary(lens_image, kwargs_result):
    ...
    for index, kwargs_mass in enumerate(kwargs_result.get('kwargs_lens', [])):
        ...
        if normalized_type == 'ELL_MPPL_OFFSET':
            for name in ('delta_varphi_m', 'phi_ref'):
                if name in kwargs_mass:
                    original[name] = float(np.degrees(np.asarray(kwargs_mass[name])))
                    units[name] = 'deg'
        elif normalized_type in {'ELL_MPPL', 'EPL_M1M3M4'}:
        ...
        if {'e1', 'e2'}.issubset(kwargs_mass):
            ...
            profile['meaning'].update({
                ...
                'axis_ratio_q': '(1 - ellipticity) / (1 + ellipticity); minor/major axis ratio.',
                'PA_deg': (
                    'Position angle measured counter-clockwise from the positive x axis; '
                    'equivalent modulo 180 deg.'
                ),
            })
        ...
        if {'gamma1', 'gamma2'}.issubset(kwargs_mass):
            ...
            converted.update({
                'gamma_ext': shear_strength,
                'PA_deg': shear_pa_deg,
            })
            profile['meaning'].update({
                'gamma_ext': 'External-shear strength sqrt(gamma1^2 + gamma2^2).',
                'PA_deg': (
                    'External-shear position angle measured counter-clockwise from the positive '
                    'x axis; equivalent modulo 180 deg.'
                ),
            })
        ...
        if normalized_type == 'ELL_MPPL_OFFSET':
            ...
            if m in {1, 3, 4}:
                ...
                converted.update({
                    ...
                    'delta_varphi_m_deg': float(np.degrees(
                        np.asarray(kwargs_mass.get('delta_varphi_m', np.nan))
                    )),
                    ...
                    'reference_profile': 'SIE (gamma fixed to 2)',
                })
                if a_m_frac is not None:
                    converted['a_m_frac'] = float(np.asarray(a_m_frac))
                profile['meaning'].update({
                    'a_m_arcsec': 'Physical SIE elliptical-multipole amplitude in arcsec.',
                    'a_m_frac': (
                        'Dimensionless fractional elliptical-radius perturbation; '
                        'a_m_arcsec = a_m_frac * r_E.'
                    ),
                    'delta_varphi_m_deg': (
                        'Eccentric anomaly from the reference ellipse semi-major axis; '
                        'it is not a polar position angle.'
                    ),
                    'phi_ref_deg': 'Reference-ellipse polar PA, counter-clockwise from +x.',
                    'axis_ratio_q': 'Axis ratio of the SIE reference ellipse.',
                    'reference_profile': 'This profile has no gamma and is defined for an SIE reference.',
                })
        elif normalized_type in {'ELL_MPPL', 'EPL_M1M3M4'}:
            m = int(round(float(np.asarray(kwargs_mass.get('m', 0)))))
            if m in {1, 3, 4}:
            ...
            if m >= 1:
                ...
                if a_m is not None and phi_m is not None:
                    ...
                    if coordinate_magnitude is not None:
                        converted['e_magnitude'] = coordinate_magnitude
                        profile['meaning']['e_magnitude'] = (
                            'MPPL coordinate magnitude sqrt(e_x^2 + e_y^2).'
                        )
                    ...
                    profile['meaning'].update({
                        ...
                        'orientation_period_deg': (
                            'Equivalent MPPL orientations repeat after this angle (360/m deg).'
                        ),
                    })
...
def save_lens_mass_ellipticity_summary(lens_image, kwargs_result, save_path):
    ...
    for profile in profiles:
        ...
        if 'ellipticity' in converted:
            q = converted['axis_ratio_q']
            q_text = 'undefined' if q is None else f'{q:.5f}'
            print(
                f"[lens_mass_parameters] {label}: e={converted['ellipticity']:.5f}, "
                f"q={q_text}, PA={converted['PA_deg']:.3f} deg"
            )
        elif 'gamma_ext' in converted:
            print(
                f"[lens_mass_parameters] {label}: gamma_ext={converted['gamma_ext']:.6g}, "
                f"PA={converted['PA_deg']:.3f} deg"
            )
        elif profile['profile'].upper() == 'ELL_MPPL_OFFSET' and 'a_m_arcsec' in converted:
            fraction_text = (
            ...
            print(
                ...
                f"delta_varphi_m={converted['delta_varphi_m_deg']:.3f} deg, "
                f"phi_ref={converted['phi_ref_deg']:.3f} deg, "
                f"q={converted['axis_ratio_q']:.5f}; SIE reference"
            )
            ...
            if amplitude_key is not None:
                m = profile['original_parameters']['m']
                phase_text = ('undefined (zero amplitude)' if converted[amplitude_key] == 0
                              else f"{converted['phi_m_deg']:.3f} deg")
                print(
                    f"[lens_mass_parameters] {label}: m={m}, "
                    f"{amplitude_key}={converted[amplitude_key]:.6g}, phi_m={phase_text} "
                    f"(period {converted['orientation_period_deg']:.3f} deg)"
                )
            elif converted:
                print(f"[lens_mass_parameters] {label}: {converted}")
            else:
                print(f"[lens_mass_parameters] {label}: {profile['original_parameters']}")
...
def _mass_ellipticity_annotation(summary):
    ...
    for profile in profiles:
        ...
        if 'ellipticity' in converted:
            ...
            lines.append(
                f"{label}: e={converted['ellipticity']:.3f}, {q_text}, "
            ...
            lines.append(
                ...
                f"delta_varphi_m={converted['delta_varphi_m_deg']:.2f} deg, "
                f"phi_ref={converted['phi_ref_deg']:.2f} deg, "
                f"q={converted['axis_ratio_q']:.3f} (SIE)"
            )
        elif profile['profile'].upper() in {'ELL_MPPL', 'EPL_M1M3M4'} and 'a_m_arcsec' in converted:
            has_elliptical_multipole = True
            fraction_text = (
            ...
            if amplitude_key is None:
                continue
            has_multipole = True
            phase_text = ('undefined' if converted[amplitude_key] == 0
                          else f"{converted['phi_m_deg']:.2f} deg")
            lines.append(
                f"{label}: {amplitude_key}={converted[amplitude_key]:.4g}, "
                f"phi_m={phase_text} (period {converted['orientation_period_deg']:.1f} deg)"
            )
    if len(lines) == 1:
        return None
    if has_multipole:
    ...
    if has_elliptical_multipole:
        lines.append(
            'ELL_MPPL_OFFSET: delta_varphi_m is an eccentric anomaly relative to the SIE '
            'reference ellipse major axis; phi_ref is its CCW polar PA from +x.'
        )
    return '\n'.join(lines)
