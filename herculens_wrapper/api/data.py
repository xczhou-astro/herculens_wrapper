...
class SingleBandData:
    ...
    @classmethod
    def from_fits(cls, image_path: str | Path, noise_path: str | Path | None, psf_path: str | Path, *, pixel_scale: float,
                  psf_supersampling_factor: int = 1,
        ...
        instance = cls(
            ...
            noise=None if noise_path is None else fits.getdata(noise_path),
        ...
        if noise_path is not None:
            instance._input_paths["noise"] = Path(noise_path).expanduser()
    ...
    def from_fits_hdus(
        ...
        *,
        ...
        with fits.open(container, memmap=False) as hdul:
            ...
            noise = None if noise_hdu is None else read_hdu(hdul, noise_hdu, "noise")
    ...
    @property
    def uses_poisson_noise(self) -> bool:
    ...
    def noise_from_model(self, model: np.ndarray, *, background_rms: float | None = None) -> np.ndarray:
        ...
        and ``exposure_time`` converts a flux rate to expected counts.
        """
        model = np.asarray(model, dtype=float)
        if model.shape != self.image.shape:
